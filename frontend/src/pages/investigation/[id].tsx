...
export default function InvestigationDetailPage() {
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      ...
      try {
        // const response = await api.getInvestigationSummary(investigationId);
        // if (response.data) {
  ...
  const handleStartInvestigation = async () => {
    ...
    await startInvestigation(currentInvestigation.id);
  };

  const handleStopInvestigation = async () => {
    if (!currentInvestigation) return;
    await stopInvestigation(currentInvestigation.id);
  };
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'COMPLETED': return <CheckCircleOutline className="h-5 w-5 text-green-500" />;
      case 'FAILED': return <Cancel className="h-5 w-5 text-red-500" />;
      case 'SCANNING':
      case 'ENRICHING': return <CircularProgress size={20} className="text-blue-500" />;
      default: return <Schedule className="h-5 w-5 text-gray-500" />;
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'FAILED': return <Badge variant="destructive">Échouée</Badge>;
      case 'SCANNING': return <Badge variant="default" className="bg-blue-100 text-blue-800">Scan en cours</Badge>;
      case 'ENRICHING': return <Badge variant="default" className="bg-yellow-100 text-yellow-800">Enrichissement</Badge>;
  ...
  const getToolIcon = (toolSource: string) => {
    switch (toolSource.toLowerCase()) {
      case 'buster': return <Search className="h-4 w-4" />;
      case 'mosint': return <Article className="h-4 w-4" />;
      case 'maigret': return <Timeline className="h-4 w-4" />;
      case 'phoneinfoga': return <ErrorOutline className="h-4 w-4" />;
      case 'spiderfoot': return <Search className="h-4 w-4" />;
      default: return <Article className="h-4 w-4" />;
    }
  };
  ...
  const transformBusterResults = (busterResults: Result[]) => busterResults.flatMap(result => result.data?.accounts || []);
  const transformEmailResults = (emailResults: Result[]) => emailResults.map(result => ({
    ...
    metadata: result.data?.metadata || { domain: '', mx_records: [], created_at: '', last_seen: '' }
  }));
  const transformUsernameResults = (usernameResults: Result[]) => usernameResults.map(result => ({
    ...
    statistics: result.data?.statistics || { total_sites: 0, found_sites: 0, success_rate: 0, by_category: {}, by_country: {}, top_platforms: [] }
  }));
  const transformPhoneResults = (phoneResults: Result[]) => phoneResults.map(result => ({
    phone_number: result.data?.phone_number || '',
    country_info: result.data?.country_info || { name: result.data?.country || '', code: '', calling_code: '' },
    carrier_info: result.data?.carrier_info || { name: result.data?.carrier || '', type: '', country: '' },
    location_info: result.data?.location_info || result.data?.location || {},
    validation: result.data?.validation || { is_valid: true, type: 'mobile' },
    risk_assessment: result.data?.risk_assessment || { score: 0, level: 'low', factors: [] },
    social_media: result.data?.social_media || [],
    metadata: result.data?.metadata || {}
  }));
  const transformComprehensiveResults = (comprehensiveResults: Result[]) => comprehensiveResults.map(result => ({
    scan_id: result.id,
    ...
    statistics: result.data?.statistics || { total_modules: 0, successful_modules: 0, failed_modules: 0, execution_time: 0 },
    risk_assessment: result.data?.risk_assessment || { overall_risk: 'low', risk_factors: [], mitigation_suggestions: [] }
  }));
  ...
  const emailResults = useMemo(() => (results || []).filter((r: Result) => r.toolSource.toLowerCase() === 'mosint'), [results]);
  const usernameResults = useMemo(() => (results || []).filter((r: Result) => r.toolSource.toLowerCase() === 'maigret'), [results]);
  const phoneResults = useMemo(() => (results || []).filter((r: Result) => r.toolSource.toLowerCase() === 'phoneinfoga'), [results]);
  const comprehensiveResults = useMemo(() => (results || []).filter((r: Result) => r.toolSource.toLowerCase() === 'spiderfoot'), [results]);
  const waybackRawResults = useMemo(() => (results || []).filter((r: Result) => r.toolSource.toLowerCase() === 'waybulk'), [results]);
  const busterResults = useMemo(() => (results || []).filter((r: Result) => r.toolSource.toLowerCase() === 'buster'), [results]);
  const darkWebResults = useMemo(() => (results || []).filter((r: Result) => r.data?.darkweb_results).flatMap((r: Result) => r.data.darkweb_results), [results]);
  const ipResults = useMemo(() => (results || []).filter((r: Result) => r.toolSource.toLowerCase() === 'asn'), [results]);
  const pdlResults = useMemo(() => (results || []).filter((r: Result) => r.toolSource.toLowerCase() === 'pdl'), [results]);
  const maigretResults = useMemo(() => (results || []).filter((r: Result) => r.toolSource.toLowerCase() === 'maigret'), [results]);

  // Mémoïsation des données transformées
  const transformedEmailData = useMemo(() => transformEmailResults(emailResults), [emailResults]);
  const transformedUsernameData = useMemo(() => transformUsernameResults(usernameResults), [usernameResults]);
  const transformedPhoneData = useMemo(() => transformPhoneResults(phoneResults), [phoneResults]);
  const transformedComprehensiveData = useMemo(() => transformComprehensiveResults(comprehensiveResults), [comprehensiveResults]);
  const waybackResults = useMemo(() => waybackRawResults.flatMap((r: Result) => Array.isArray(r.data) ? r.data.map((url: string) => ({ url })) : []), [waybackRawResults]);
  const socialProfiles = useMemo(() => transformBusterResults(busterResults), [busterResults]);
  ...
  if (isLoading && !currentInvestigation) {
    return (
      <SidebarProvider defaultOpen={false}>
        ...
        <SidebarInset><div className="flex items-center justify-center h-screen"><CircularProgress size={32} /></div></SidebarInset>
      </SidebarProvider>
    );
  }
      ...
      <SidebarProvider defaultOpen={false}>
        ...
        <SidebarInset>
          <div className="flex flex-col items-center justify-center h-screen">
            ...
            <Button onClick={() => router.push('/investigations')}><ArrowBack className="h-4 w-4 mr-2" />Retour à la liste</Button>
          </div>
        </SidebarInset>
      </SidebarProvider>
    );
    ...
    <SidebarProvider defaultOpen={false}>
      ...
      <SidebarInset>
        <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
          <div className="px-4 lg:px-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <Button variant="ghost" size="sm" onClick={() => router.push('/investigations')}><ArrowBack className="h-4 w-4 mr-2" />Retour</Button>
                <div>
                  <h1 className="text-3xl font-bold tracking-tight">Investigation {currentInvestigation.id}</h1>
                  <p className="text-muted-foreground">Créée le {formatDate(currentInvestigation.createdAt)}</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" size="sm"><FileDownload className="h-4 w-4 mr-2" />Exporter</Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent>
          ...
          <div className="px-4 lg:px-6">
            <Card><CardHeader><CardTitle>Journal de l'investigation</CardTitle></CardHeader><CardContent><ModernInvestigationTimeline logs={logs || []} /></CardContent></Card>
          </div>
          ...
          {isAnalysisRunning && (
            <div className="px-4 lg:px-6">
              <Alert className="border-blue-500 bg-blue-50 text-blue-800">
                ...
                <AlertDescription>Les résultats s'affichent en temps réel. De nouvelles informations peuvent apparaître à tout moment.</AlertDescription>
              </Alert>
            </div>
          )}
          ...
          <div className="px-4 lg:px-6">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
              <TabsList className="grid w-full grid-cols-12">
                ...
                <TabsTrigger value="results">Résultats Bruts ({results?.length || 0})</TabsTrigger>
                <TabsTrigger value="email-analysis"><Email className="h-4 w-4 mr-1" />Emails ({emailResults.length})</TabsTrigger>
                <TabsTrigger value="username-analysis"><Person className="h-4 w-4 mr-1" />Usernames ({usernameResults.length})</TabsTrigger>
                <TabsTrigger value="maigret-profiles"><Person className="h-4 w-4 mr-1" />Maigret ({maigretResults.length})</TabsTrigger>
                <TabsTrigger value="phone-analysis"><PhoneIcon className="h-4 w-4 mr-1" />Téléphones ({phoneResults.length})</TabsTrigger>
                <TabsTrigger value="comprehensive-analysis"><Language className="h-4 w-4 mr-1" />Complet ({comprehensiveResults.length})</TabsTrigger>
                <TabsTrigger value="wayback-archives"><Article className="h-4 w-4 mr-1" />Archives ({waybackResults.length})</TabsTrigger>
                <TabsTrigger value="buster-results"><Search className="h-4 w-4 mr-1" />Buster ({socialProfiles.length})</TabsTrigger>
                <TabsTrigger value="dark-web"><Warning className="h-4 w-4 mr-1" />Dark Web ({darkWebResults.length})</TabsTrigger>
                <TabsTrigger value="graph"><Timeline className="h-4 w-4 mr-1" />Graphe</TabsTrigger>
                <TabsTrigger value="ip-analysis"><Public className="h-4 w-4 mr-1" />IPs ({ipResults.length})</TabsTrigger>
                <TabsTrigger value="pdl-profiles"><Person className="h-4 w-4 mr-1" />PDL ({pdlResults.length})</TabsTrigger>
                <TabsTrigger value="logs">Logs ({logs?.length || 0})</TabsTrigger>
                <TabsTrigger value="report" disabled={currentInvestigation.status !== 'COMPLETED'}><Article className="h-4 w-4 mr-1" />Rapport</TabsTrigger>
              </TabsList>

              <TabsContent value="summary" className="space-y-4">
                <Card><CardHeader><CardTitle>Résumé des Données Trouvées</CardTitle></CardHeader>
                  <CardContent>
                    {summaryLoading ? <div className="flex justify-center items-center h-48"><CircularProgress /></div> : <UnifiedResultsTable items={summaryItems} />}
                  </CardContent>
                </Card>
              </TabsContent>
              ...
              <TabsContent value="indicators" className="space-y-4">
                <Card><CardHeader><CardTitle>Indicateurs analysés</CardTitle></CardHeader>
                  <CardContent>
                    {indicators.length === 0 ? <p className="text-muted-foreground">Aucun indicateur trouvé pour le moment...</p> : (
                      <div className="space-y-2">
                        {indicators.map((indicator: Indicator) => (
                          <div key={indicator.id} className="flex items-center justify-between p-3 border rounded-lg">
                            <div><p className="font-medium">{indicator.value}</p><p className="text-sm text-muted-foreground">Type: {indicator.type}</p></div>
                            <div className="flex items-center gap-2"><Badge variant={indicator.verified ? "default" : "outline"}>{indicator.verified ? "Vérifié" : "Non vérifié"}</Badge><Badge variant="outline">Confiance: {indicator.confidence}%</Badge></div>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              <TabsContent value="results" className="space-y-4">
                <Card><CardHeader><CardTitle>Résultats bruts</CardTitle></CardHeader>
                  <CardContent>
                    {(results?.length || 0) === 0 ? <p className="text-muted-foreground">Aucun résultat trouvé pour le moment...</p> : (
                      <div className="space-y-2">
                        {(results || []).map((result: Result) => (
                          <div key={result.id} className="flex items-center justify-between p-3 border rounded-lg">
                            <div className="flex items-center gap-3">{getToolIcon(result.toolSource)}<div><p className="font-medium">{result.toolSource}</p><p className="text-sm text-muted-foreground">Score: {result.score}</p></div></div>
                            <div className="text-right"><p className="text-sm text-muted-foreground">{formatDate(result.createdAt)}</p></div>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              <TabsContent value="email-analysis" className="space-y-4">
                <Card className={analysisCardClass}><CardHeader><CardTitle>Analyse des emails (Mosint)</CardTitle></CardHeader>
                  <CardContent>
                    {emailResults.length === 0 ? <p className="text-muted-foreground">Aucun résultat d'email disponible pour le moment...</p> : <EmailAnalysisView data={transformedEmailData} investigationId={investigationId || ''} />}
                  </CardContent>
                </Card>
              </TabsContent>

              <TabsContent value="username-analysis" className="space-y-4">
                <Card className={analysisCardClass}><CardHeader><CardTitle>Analyse des noms d'utilisateur (Maigret)</CardTitle></CardHeader>
                  <CardContent>
                    {usernameResults.length === 0 ? <p className="text-muted-foreground">Aucun résultat de nom d'utilisateur disponible pour le moment...</p> : <UsernameAnalysisView data={transformedUsernameData} />}
                  </CardContent>
                </Card>
              </TabsContent>

              <TabsContent value="maigret-profiles" className="space-y-4">
                <Card className={analysisCardClass}><CardHeader><CardTitle>Profils trouvés (Maigret)</CardTitle></CardHeader>
                  <CardContent>
                    {usernameForRecursiveSearch && investigationId ? <ProfilesGrid profiles={maigretProfilesForGrid} investigationId={investigationId} username={usernameForRecursiveSearch} /> : <p className="text-muted-foreground">{maigretProfilesForGrid.length > 0 ? "Données de l'investigation manquantes pour activer les actions." : "Aucun profil Maigret trouvé pour le moment."}</p>}
                  </CardContent>
                </Card>
              </TabsContent>

              <TabsContent value="phone-analysis" className="space-y-4">
                <Card className={analysisCardClass}><CardHeader><CardTitle>Analyse des téléphones (PhoneInfoga)</CardTitle></CardHeader>
                  <CardContent>
                    {phoneResults.length === 0 ? <p className="text-muted-foreground">Aucun résultat de téléphone disponible pour le moment...</p> : <PhoneAnalysisView data={transformedPhoneData} />}
                  </CardContent>
                </Card>
              </TabsContent>

              <TabsContent value="comprehensive-analysis" className="space-y-4">
                <Card className={analysisCardClass}><CardHeader><CardTitle>Analyse complète (SpiderFoot)</CardTitle></CardHeader>
                  <CardContent>
                    {comprehensiveResults.length === 0 ? <p className="text-muted-foreground">Aucun résultat complet disponible pour le moment...</p> : <ComprehensiveReportView data={transformedComprehensiveData} />}
                  </CardContent>
                </Card>
              </TabsContent>

              <TabsContent value="wayback-archives" className="space-y-4">
                <Card className={analysisCardClass}><CardHeader><CardTitle>Archives du site (Wayback Machine)</CardTitle></CardHeader>
                  <CardContent>
                    {waybackResults.length === 0 ? <p className="text-muted-foreground">Aucune archive trouvée pour le moment...</p> : <WaybackResults results={waybackResults} />}
                  </CardContent>
                </Card>
              </TabsContent>
              ...
              <TabsContent value="buster-results" className="space-y-4">
                <div className="space-y-4">
                  <Card className={analysisCardClass}><CardHeader><CardTitle>Profils Sociaux (Buster)</CardTitle></CardHeader>
                    <CardContent>
                      {socialProfiles.length === 0 ? <p className="text-muted-foreground">Aucun profil social trouvé pour le moment...</p> : <SocialProfiles profiles={socialProfiles} />}
                    </CardContent>
                  </Card>
                  <Card className={analysisCardClass}><CardHeader><CardTitle>Reverse Whois (Buster)</CardTitle></CardHeader>
                    <CardContent><ReverseWhoisResults domains={reverseWhoisDomains} /></CardContent>
                  </Card>
                </div>
              </TabsContent>

              <TabsContent value="logs" className="space-y-4">
                <RealTimeNotifications logs={logs || []} investigationId={currentInvestigation.id} />
              </TabsContent>
              ...
              <TabsContent value="ip-analysis" className="space-y-4">
                <IpAnalysisView data={ipResults.map((r: Result) => r.data)} />
              </TabsContent>

              <TabsContent value="pdl-profiles" className="space-y-4">
                {pdlResults.map((result: Result, i: number) => (<PersonProfileView key={i} profile={result.data} />))}
              </TabsContent>

              <TabsContent value="report" className="space-y-4">
                <InvestigationReportView investigation={currentInvestigation} results={results || []} indicators={indicators} />
              </TabsContent>
            </Tabs>
          </div>
        </div>
